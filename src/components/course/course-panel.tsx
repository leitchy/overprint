...
export function CoursePanel({
  ...
  // Calculate course length only when we have an active course
  const lengthMetres =
    course && mapFile
  ...
  return (
    <div className="absolute right-4 top-4 w-48 rounded bg-white/90 shadow">
      {/* Course tab switcher */}
      <CourseTabs />

      {/* Course stats — only when a course is active */}
      {course && (
      ...
      {/* Control list — only when active course has controls */}
      {course && courseId && (
